...
// @desc    Check if a token is valid (for debugging)
// @access  Public
router.get('/check-token/:token', async (req, res) => {
    try {
      ...
      // Find all users and check their tokens
      ...
      if (usersWithTokens.length > 0) {
        usersWithTokens.forEach(user => {
          ...
          console.log('User token:', user.emailVerificationToken);
          console.log('Token matches?', user.emailVerificationToken === hashedToken);
      ...
      if (user) {
        ...
        if (user.emailVerificationExpire < Date.now()) {
          console.log('Token is expired. Expires at:', user.emailVerificationExpire);
          console.log('Current time:', Date.now());
          return res.json({ 
            valid: false, 
            reason: 'expired',
            expires: user.emailVerificationExpire,
            now: Date.now()
          });
        }
        ...
        return res.json({ valid: false, reason: 'no-user' });
      }
    } catch (error) {
